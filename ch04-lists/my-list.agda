...
-- view 𝕃 as a function which takes in atype and returns a type. (type-level function)
-- We will allow 𝕃 to take in a type at any type level ℓ. It will return a type at that same level (required by Agda).
...
-- Anonymous functions can't be recursive in Agda.
