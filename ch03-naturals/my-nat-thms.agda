...
-- The goal (proof) is (suc x) + 0 ≡ suc x
-- We can simplify using the definition of _+_
-- The goal now is suc (x + 0) ≡ suc x
-- Then, we rewrite (x + 0) ≡ x by using a recursive call to the proof.
-- Getting the goal suc x ≡ x which is trivial to proof.
--
-- Recursive proofs are called proofs by induction in mathematics.
+0 : ∀ (x : ℕ) → x + 0 ≡ x
+0 zero = refl
...
--
-- Rule of thumb:
--   1. Pick the one being pattern-matched on the recursive call
--   2. If many, pick the one where pattern-matching will happen more.
--
-- Since addition does pattern matching on the first argument, it is a good idea to consider induction on a variable used as the first argument to addition: x and y
--
-- x is used twice, while y is ued only once.
--
-- So we will use x
...
-- We only need to proof x + (y + z) ≡ x + y + z
-- But you can achieve that using an inductive proof.
